...
function summarizePromptInstallError(error) {
...
export async function autoInstallAgentPrompt(options = {}) {
    ...
    try {
        ...
        safeLog(() => options.logger?.warn?.(`[libp2p-mesh] Failed to install AGENTS.md prompt automatically: ${summarizePromptInstallError(error)}`));
        return;
    }
    safeLog(() => options.logger?.info?.(result.existed

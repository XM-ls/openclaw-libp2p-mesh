...
function firstAttemptedResult(results) {
    return (results.find((result) => result.ok) ??
        results.find((result) => isNonEmptyString(result.channel) && isNonEmptyString(result.target)));
}
export function createInstanceRouter(options) {
    ...
    async function announceToConnectedPeers() {
        for (const peerId of mesh.getConnectedPeers()) {
            await announceToPeer(peerId).catch((error) => {
                logger?.warn?.(`[libp2p-mesh] Failed to announce to ${peerId}: ${summarizeError(error)}`);
            });
        }
    }
    async function handleAnnounce(msg) {
        ...
        if (msg.instanceId !== payload.instanceId || payload.peerId !== msg.from) {
            logger?.warn?.(`[libp2p-mesh] Ignoring instance announce with mismatched envelope from ${msg.from}`);
            return;
        }
    ...
    async function handleUserMessage(msg) {
        ...
        if (msg.instanceId !== payload.fromInstanceId) {
            logger?.warn?.(`[libp2p-mesh] Ignoring user-message with mismatched instance envelope from ${msg.from}`);
            return;
        }
    ...
    function handleAck(msg) {
        ...
        if (pending.peerId !== msg.from) {
            logger?.warn?.(`[libp2p-mesh] Ignoring delivery ACK for ${payload.ackFor} from unexpected peer ${msg.from}`);
            return;
        }
    ...
    async function sendInstanceMessage(instanceId, message) {
